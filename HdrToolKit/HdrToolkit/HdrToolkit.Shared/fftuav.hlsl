...
#ifdef BUTTERFLY_LUT
Texture2D<float4> TextureButterfly  : register(t2);
#endif
...
void Butterfly(uint3 position : SV_DispatchThreadID)
{
#ifdef ROWPASS
	...
	float2 Weights;
#ifdef BUTTERFLY_LUT
	float4 IndicesAndWeights = TextureButterfly[uint2(textureSampleX, PassIndex)];
