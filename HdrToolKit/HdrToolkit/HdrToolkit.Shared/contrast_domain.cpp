...
static void pyramid_show(pyramid_t *pyramid)
{
	...
	while (pyramid != NULL)
	{
		...
		ss = "Gx " + std::to_string(reinterpret_cast<size_t>(pyramid->Gx)) + " ";
		if (pyramid->Gx != NULL)
			matrix_show(ss.c_str(), pyramid->cols, pyramid->rows, pyramid->Gx);
		ss = "Gy " + std::to_string(reinterpret_cast<size_t>(pyramid->Gy)) + " ";
		if (pyramid->Gy != NULL)
			matrix_show(ss.c_str(), pyramid->cols, pyramid->rows, pyramid->Gy);
	...
	for (int y = 0; y < outRows; y++)
	{
		...
		for (int x = 0; x < outCols; x++)
		{
			...
			out[x + y * outCols] = (((ix1 + 1) - sx) * ((iy1 + 1 - sy)) * in[ix1 + iy1 * inCols] +
				...
				(sx + dx - (ix1 + 1)) * (sy + dx - (iy1 + 1)) * in[ix2 + iy2 * inCols]) *
				factor;
		}
	}
}
...
#if defined(_DEBUG)
		OutputDebugStringW(("\nIteration: " + (iter + 1) + ", error: " + sqrtf(err2 / bnrm2) + " = sqrt(" + err2 + "/" + bnrm2 + ")\n")->Data());
#endif
		if (err2 / bnrm2 < tol2)
			break;
	}
...
#pragma omp parallel for schedule(static)
	for (int j = 0; j < n; j++)
	{
		if (unlikely(R[j] < clip_min))
			R[j] = clip_min;
		if (unlikely(G[j] < clip_min))
			G[j] = clip_min;
		if (unlikely(B[j] < clip_min))
			B[j] = clip_min;
		if (unlikely(Y[j] < clip_min))
			Y[j] = clip_min;
	}
